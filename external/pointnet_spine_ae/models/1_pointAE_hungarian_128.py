...
def perm_to_best_match(x,y):
    ...
    for batchIdx in range(x.shape[0]):
        ...
        cost_matrix = distance.cdist(x[batchIdx], y[batchIdx])
        #optimum = Munkres(cost[0])
        m = Munkres()
        indexes = m.compute(cost_matrix)
        print("Size y and indexes")
        print(y.shape)
        print(indexes)
        #for i in indexes:
            #outBatch[i] = y[batchIdx, np.where(optimum[i])[0][0]]
        #    outBatch[i] = y[batchIdx][i]
        for tup in indexes:
            outBatch[tup[0],:] = y[batchIdx, tup[1],:]
        out[batchIdx] = outBatch
    #print(np.square(np.subtract(x, out)).sum())
    return out.astype(np.float32)
    
def get_loss(pred, label, end_points):
    goal_pcs2 = tf.py_func(perm_to_best_match, [pred,label], tf.float32)
    loss = tf.reduce_sum(tf.square(tf.subtract(pred, goal_pcs2)))
